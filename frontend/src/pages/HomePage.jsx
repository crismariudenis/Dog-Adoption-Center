...
function AdoptModal({ dog, onClose }) {
    ...
    setForm((current) => {
      if (
        current.documents.some(
          (document) => document.documentType === "LandlordConsent",
        )
      ) {
        return current;
      }
  ...
  const validateApplication = async () => {
    ...
    try {
      ...
      const idDocument = form.documents.find(
        (document) => document.documentType === "GovernmentId",
      )?.file;
      const bankStatement = form.documents.find(
        (document) => document.documentType === "ProofOfResidence",
      )?.file;

      if (!idDocument || !bankStatement) {
        setErrorMsg(
          "Upload both GovernmentId and ProofOfResidence files to run extraction.",
        );
        return;
      }
      ...
      setErrorMsg(
        err?.message ||
          "Validation failed. Check the document fields and try again.",
      );
      console.error(err);
    }
  };
        ...
        {success ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {validation && (
                <div
                  className={`rounded-lg p-3 text-sm ${validation.isValid ? "bg-green-50 text-green-800" : "bg-red-50 text-red-800"}`}
                >
              ...
              {extraction && (
                <div className="rounded-lg p-3 text-sm bg-blue-50 text-blue-900 space-y-2">
                  ...
                  <p>
                    ...
                    {extraction.extractedIdName || "Not found"}
                  </p>
                  {extraction.idImagePreviewBase64 && (
                    <img
                      src={extraction.idImagePreviewBase64}
                      alt="Extracted ID preview"
                      className="w-40 h-auto rounded border border-blue-200"
                    />
                  )}
                  <p>
                    <strong>Bank Balance:</strong>{" "}
                    {extraction.extractedBankBalance != null
                      ? `${extraction.extractedBankBalance} ${extraction.extractedBankCurrency || ""}`
                      : "Not found"}
                  </p>
                  <p>
                    <strong>Bank Country:</strong>{" "}
                    {extraction.extractedBankCountry || "Not found"}
                  </p>
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1">
                  Why do you want to adopt {dog.name}?
                </label>
                <textarea
                  required
                  value={form.message}
              ...
              <div className="rounded-xl border border-amber-100 bg-amber-50/60 p-3 space-y-3">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-amber-700">
                      This is only validated locally and is not stored.
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                <label className="flex items-center gap-2 text-sm text-gray-700">
                  <input
                    ...
                    onChange={(e) =>
                      setForm((current) => ({
                        ...current,
                        requiresLandlordConsent: e.target.checked,
                      }))
                    }
                  />
                ...
                {form.documents.map((document, index) => (
                  <div
                    ...
                    className="grid grid-cols-1 gap-2 rounded-lg bg-white p-3 border border-amber-100"
                  >
                    ...
                    <p className="text-xs text-gray-500">
                      {document.file
                        ? `Selected: ${document.file.name} (${document.file.type || "unknown type"})`
                        : "No file selected"}
                    </p>
                  </div>
                ))}
            ...
            >
              ...
              <div className="p-4">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span
                    className={`text-xs font-medium px-2 py-0.5 rounded-full ${statusStyle[dog.status]}`}
                  >
                    {dog.status}
                  </span>
                </div>
